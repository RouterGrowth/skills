...
5. **Recover within budget.** Broaden equivalent titles once and remove only optional filters. Read the run's `hints`, error and `attempts`: a pinned provider never falls back; `auto` still needs a cap covering an alternative's complete quote. A $0.036 cap can allow a single database result but excludes a company-constrained LinkedIn full-profile search (one profile at one employer quotes $0.186075 at list price). Use inspect and the returned quote guidance; do not raise a user-set budget without authorization. A higher cap permits another attempt; it does not guarantee a match.
