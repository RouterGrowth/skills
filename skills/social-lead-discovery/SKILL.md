...
Each post comes back with `author` (the display name), `author_handle`, `author_url` when the platform gives one, `url`, `text`, `created_at` and `engagement`. Priced per post, about $0.0006 on X, $0.003 on Instagram, $0.0026 on TikTok and $0.0036 on YouTube at the time of writing, so 30 posts on X is 2 cents and 50 on Instagram is under 20 cents. The quote is the requested limit and it is the floor: 20 posts back on a limit of 30 still bill 30. Three phrases per platform beats one broad one: the specific phrase is where the specific buyer posts. Expect noise on X, a third to a half of the rows: that is what step 2 is for, and it is free.
