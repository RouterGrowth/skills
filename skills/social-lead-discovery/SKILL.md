...
Each post comes back with `author` (the display name), `author_handle`, `author_url` when the platform gives one, `url`, `text`, `created_at` and `engagement`. Priced per post, about $0.0007 on X, $0.004 on Instagram, $0.00525 on YouTube and $0.00175 a search plus $0.00525 a post on TikTok at the time of writing, so 30 posts on X is 2 cents and 50 on Instagram is about 20 cents. The hold covers the requested limit and the charge covers the posts returned: 20 posts back on a limit of 30 bill 20. Three phrases per platform beats one broad one: the specific phrase is where the specific buyer posts. Expect noise on X, a third to a half of the rows: that is what step 2 is for, and it is free.
...
Estimate from the rate card at the time of writing, not a recorded run: three platforms at 50 posts each, about $0.72; 60 distinct authors qualified down to 30 profiles, about $0.16; 12 of those with an email in the profile; 10 with a domain sent to `contact.find`, at most $0.44 with the misses free; 20 verifications, about $0.22. About $1.55 for roughly 20 verified contacts and a social-only list of the rest. Instagram business accounts carry an email far more often than TikTok or YouTube accounts do, so a run that is Instagram-heavy spends less on step 4.
