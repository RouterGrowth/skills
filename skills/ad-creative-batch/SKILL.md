...
Prices when this was written: image $0.0045 (flux-schnell, per image), edit $0.0585, cutout $0.027, upscale $0.18 (2x) or $0.72 (4x), video $0.0675 a second on hailuo-02 ($0.405 for its 6-second clip; other models cost more, inspect lists them). Charges can settle a few percent above the quote (a larger source image, a longer clip), so set `max_cost` above the quote, not equal to it. The upscale and the video are the surprise lines; quote them explicitly. Cutouts and upscales return `result.image`, not a list. If the CLI drops its connection while waiting on a slow run, the run keeps going on the router: `routergrowth runs` shows it and `runs get` fetches the result. Do not resubmit before checking.
